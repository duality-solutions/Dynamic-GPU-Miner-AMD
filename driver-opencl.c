  ...
  int64_t __maybe_unused max_nonce)
{
  ...
  else if (gpu->algorithm.type == ALGO_ARGON2D)
  {
	  const uint32_t throughput = gpu->throughput;


	  const size_t global[2] = { 16, throughput };
	  ...
	  const size_t global2[2] = { 32 * 8, throughput };
	  const size_t local2[2] = { 32 * 8, 1 };
	  status = clEnqueueNDRangeKernel(clState->commandQueue, clState->extra_kernels[0], 2, NULL, global2, local2, 0, NULL, NULL);
	  ...
	  status = clEnqueueNDRangeKernel(clState->commandQueue, clState->extra_kernels[1], 2, NULL, global3, local3, 0, NULL, NULL);
